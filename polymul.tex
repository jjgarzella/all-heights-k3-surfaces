...
implement due to its implementation difficulty and potential marginal 
speedup.
...
polynomial $f'$ in our input space, the polynomial with all possible 
...
power, and solve for $p_2$ as the remainder of the division of 
$k \div p_1$. Note that this computation is relatively expensive, 
taking a few minutes per power, but it only needs to be ran once, and 
$p_1$, $p_2$ and $i$ can be hard coded for any problem of the same shape.
Thus, given a problem shape, \code{steps} is hard coded to return the 
