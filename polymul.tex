...
We use the 4-step FFT \cite{bailey-1989-fft} with Barrett Modular 
Reduction, for our NTT algorithm.
State-of-the-art implementations often involve pre-computing 
a table of twiddle factors to avoid an expensive powermod. 
We choose to omit this, since having tables of twiddle factors 
for both the forward NTT and inverse NTT effectively triples 
the required GPU memory, which becomes the bottleneck for very large 
problems.

For ease of implementation, our 
NTT length $L$ is always a power of $2$. 
For each NTT we perform, we search for primes $p$ that satisfy $p \equiv 1 \mod L$, compute primitive $L$th roots of unity for each $p$, and $L^{-1} \mod p$. 
...
the GPU one by one, applying the FFT, and move them back to the CPU where 
the chinese remainder theorem recovery is performed.
...
Experimentally, NTTs with 32-bit integers are slightly over double 
the speed of NTTs with 64-bit integers. Thus, for prime selection, 
we ideally choose primes satisfying $p \equiv 1 \mod L$ that fit 
within 32 bits. 

When the upper bound $M$ for the resulting coefficients is too high, and 
the NTT size is too large, we start to run out of distinct 32-bit NTT primes.
In this case, we switch to 64-bit integers, which hold enough primes for any 
reasonably computable NTT on the GPU.
...
\begin{defn}
    Given a basis $(\mathbf{x}^{I_1}, \dots, \mathbf{x}^{I_n})$, where $I_i$ are distinct degree sequences of equal length, the \textbf{maximum polynomial} is $(m - 1)(\mathbf{x}^{I_1} + \cdots + \mathbf{x}^{I_n})$, where $m$ is a non-inclusive upper bound on the coefficients.
\end{defn}
...
for an upper bound. To get the optimal upper bound, we can plug in the
