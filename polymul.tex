...
is implemented. Specifically, we focus on Step 2, 
...
% Maybe a subsubsection about what the NTT is?
...
is injective on the subset of elements 
...
The traditional method of raising a polynomial in $\mathbb{Z}[x]$ 
to a power using the NTT is to compute the power modulo multiple primes, 
then use the Chinese Remainder Theorem to obtain the result in 
$\mathbb{Z}[x]$, as described in \cite{monagan-2012-sparse-powering}.
This method, known as the multimodular NTT, is competitive 
for medium-sized problems, like 
computing the quasi-$F$-split height for a K3 surface over 
$\mathbb{F}_5$ or $\mathbb{F}_7$. However, for larger problems, 
large coefficients require the use of many primes. Combining this 
with large NTT sizes makes GPU DRAM size a bottleneck, 
forcing us to perform some sort of paging to CPU RAM, severely 
slowing down the computation.

To address this, we can exploit the fact that we are powering 
polynomials in $(\mathbb{Z}/p^2\mathbb{Z})[x]$ instead of $\mathbb{Z}[x]$. 
If we compute the power in smaller increments by repeatedly 
multiplying to an accumulator, we can reduce coefficients $\mod p^2$ 
after each step, and completely avoid the consequences of having 
extremely large coefficients. In fact, even for large problems like 
computing the quasi-$F$-split height for K3 surfaces over 
$\mathbb{F}_{11}$ or $\mathbb{F}_{13}$, we can get away with only using 
one prime for the entire computation.

\begin{algorithm}[H]
    \caption{Incremental polynomial powering}
    \label{alg:poly:incr}
    \KwInput{$f \in (\mathbb{Z}/m_1\mathbb{Z})[x], k \in \mathbb{Z}, 
    m_1, m_2 \in \mathbb{Z}$}
    \KwOutput{$\tilde{f}^k \in (\mathbb{Z}/m_2\mathbb{Z})[x]$}
    $p_1, p_2, i \gets \code{steps}()$\;
    $vec \gets \code{dense}(f)$\;
    $\widehat{vec} \gets \code{NTT}(vec)$\;
    \Comment{Compute NTT of $f^{p_1} ~\%~ m_2$ and 
    store in temporary array}
    $\widehat{aux} \gets \widehat{vec}^{p_1}$\;
    $aux \gets \code{INTT}(\widehat{aux})$\;
    $aux \gets aux ~\%~ m_2$\;
    $\widehat{aux} \gets \code{NTT}(aux)$\;
    \Comment{Compute $f^{p_2} ~\%~ m_2$ and store in accumulator}
    $\widehat{vec} \gets \widehat{vec}^{p_2}$\;
    $vec \gets \code{INTT}(\widehat{vec})$\;
    $vec \gets vec ~\%~ m_2$\;
    \Comment{Start multiplying to accumulator}
    \For{$\_ \in \{1, ... , i\}$}{
        $\widehat{vec} \gets \code{NTT}(vec)$\;
        $\widehat{vec} \gets \widehat{vec} \times \widehat{aux}$\;
        $vec \gets \code{INTT}(\widehat{vec})$\;
        $vec \gets vec ~\%~ m_2$
    }
    $\tilde{f}^k \gets \code{sparse}(vec)$\;
    \Return $\tilde{f}^k$
\end{algorithm}

The $\code{steps}$ function determines what intermediate powers 
are used in the computation, satisfying $k = i \cdot p_1 + p_2$.
The exact semantics and use of the $\code{steps}$ function are 
given in section \ref{subsec:poly:bounds}. The $\code{dense}$ and 
$\code{sparse}$ functions use the Kronecker substitution to map a 
multivariate polynomial to a dense univariate one, as described 
in the previous section.

The initial $f^{p_1}$ and $f^{p_2}$ computations can be done with 
a smaller NTT size, amortizing their cost compared to the large 
NTTs that need to be performed in the loop. The NTT size within the 
loop stays constant, because otherwise for each iteration we would 
need to compute the NTT and INTT of $aux$.

\subsubsection{Goldilocks}

Because we only use one prime for all of our NTTs, we can achieve 
significant performance gains from using the special prime 
$q = \Phi_6(2^{32}) = 2^{64} - 2^{32} + 1$, known as the Goldilocks 
prime. Goldilocks is special because reducing a 128-bit integer, 
which shows up in modular multiplications inside of the NTT, requires 
fewer instructions when compared to other modular reduction 
techniques like Barrett or Montgomery reduction. The algorithm to 
reduce modulo Goldilocks is given in \cite{ozcan-2023-fft}. 
% Maybe reword this sentence, but think it is important enough to mention
Note that 
the fact that $2^{32}$ is a primitive 6th root of unity in $\mathbb{F}_q$ 
can be used to slightly speed up the forward NTT, which we don't 
implement due to its implementation difficulty, the fact that we don't need it, and 
our expectations that the gains will be marginal.

\subsubsection{Bound Finding}
\label{subsec:poly:bounds}

Ideally, we'd like to perform as few multiplications as possible, 
meaning we should be greedy with breaking the power down into 
increments as large as possible. Our only limitation is that we don't 
want the resulting coefficients of any polynomial operation to 
overflow Goldilocks. There are two types of computations that we need to 
worry about overflowing Goldilocks - $f^{p_1}$ and $f^{p_2}$, and the 
intermediate multiplications.

Recall that $p_1$ and $p_2$ are the intermediate powers we raise our 
polynomial to, satisfying $i \cdot p_1 + p_2 = k$, where $i$ is the 
number of loop iterations. We essentially want $p_1$ to be as large 
as possible to minimize loop iterations, and $p_2$ to be the remaining 
power of $f$ to reach $f^k$. With this goal, $p_1 > p_2$.

To find how large we can make $p_1$, we take the ``worst case'' 
polynomial $f'$ (i.e. the polynomial whose coefficients are all equal to \(p^{2}-1\)) in our input space, the polynomial with all possible 
terms, and all coefficients taking on their maximum values. Then, using 
an arbitrary precision computer algebra system like FLINT, 
we raise $f'$ to higher and higher powers 
until we reach a power where the largest resulting coefficient is 
larger than Goldilocks. We can then take $p_1$ to be the previous 
power, and solve for $p_2$ as the remainder 
\(k \text{ mod } p_{1}\).
% of the division of 
% $k \div p_1$. 
Note that this computation is relatively expensive, 
taking a few minutes per power, but it only needs to be run once, and 
$p_1$, $p_2$, and $i$ can be stored for any problem of the same shape.
Thus, given a problem shape, \code{steps} simply returns the 
corresponding $(p_1, p_2, i)$.

For the intermediate multiplications, we can use the trivial 
bound that convolving two 
$n$-length vectors with maximum coefficient $m$ results in a maximum 
coefficient of $m^2n$. This comes from the fact that 
multiplying two univariate $n-1$ degree polynomials with maximum 
coefficients $m$ results in the $m^2n$ upper bound on the resulting 
coefficients. Because our largest 
FFT length ends up being $2^{28}$ for the quasi-$F$-split height of a 
K3 surface over $\mathbb{F}_{13}$ problem, and we are working mod $p^2$,
this gives us the upper bound $2^{28} \cdot 12^2$, which is less than 
Goldilocks.
...
Here, we briefly comment on how these algorithms perform on the GPU.
...
% expensive CPU multiplication to set up the algorithm.
...
% \begin{algorithm}[H]
%     \caption{$\Delta_1$ via lifting to $\mathbb{Z}/p^{2}\mathbb{Z}$}
%     \label{alg:polymul:idk}
%     \KwInput{$\Delta, B_d, B_{d^\prime}, p$}
