...
computing the quasi-$F$-split height for a $K_3$ surface over 
...
computing the quasi-$F$-split height for $K_3$ surfaces over 
...
Ideally, we'd like to perform as little multiplications as possible, 
...
taking a few minutes per power, but it only needs to be ran once, and 
...
$K_3$ surface over $\mathbb{F}_{13}$ problem, and we are working mod $p^2$,
