...
\begin{defn}
	...
	\[
		\omega_{n}(X_{0}, \ldots, X_{n}) = X_{0}^{p^{n}} + pX_{1}^{p^{n-1}} + \cdots + p^{n}X_{n}
...
% \] 
\begin{align*}
    \Phi : \prod_{n \in \mathbb{N}}^{} R  
    &\xlongrightarrow{(\omega_{n})_{n}} 
    \prod_{n \in \mathbb{N}}^{} R \\
    %
    (r_{0}, r_{1}, \ldots, r_{n}, \ldots) 
    &\xmapsto{\hphantom{(\omega_{n})_{n}}}
    (\omega_{0}(r_{0}), \omega_{1}(r_{0}, r_{1}), 
    \ldots, \omega_{n}(r_{0}, \ldots, r_{n}), \ldots).
\end{align*}

where the \(n\)-th component is defined by \(\omega_{n}\).
That is, \(\omega_{n}\) is considered as a 
% \omega_n takes n+1 arguments, so R^{n + 1}
map (of sets) \(R^{n + 1} \xrightarrow{} R\) by
evaluation.
...
\begin{rmk}
    ...
    \begin{enumerate}[(1)]
    	\item Let \(R\) be reduced. 
            We may formally identify \(R\) with the set \(R^{p}\)
            of symbols \(\{r^{p} : r \in R\}\),
            which inherits the ring structure from \(R\).
            Then, the Frobenius map identifies as
            \(r^{p} \mapsto r^{p}\), where the former
            ``to the \(p\)'' is just a symbol, and the
            latter comes from multiplication in the ring \(R\).
            Since \(R\) is reduced, this map is an injection,
            so we may identify \(R^{p}\) with this image.
    		Thus, we may view the Frobenius as the inclusion
    		\(R^{p} \ins R\).
    	\item 
            Similarly, again assuming that \(R\) is reduced
...
\begin{defn}
    ...
    That is, there exists a map \(\sigma : F_{\star}R \xrightarrow{} R\) 
    such that \(\sigma \circ F = id\).
\end{defn}
...
map is module-finite (that is, finitely generated as a module).
...
then \(I^{[p^{e}]}\)
is defined to be \((x_{1}^{p^{e}}, \ldots, x_{n}^{p^{e}})\).
...
One may observe that if \(R\) is \(n\)-quasi-\(F\)-split,
then it is \(n+1\)-quasi-\(F\)-split, by composing \(\phi\)
with the truncation map \(W_{n+1}(R) \xrightarrow{} W_{n}(R)\)
(this uses that the Witt vector Frobenius commutes with truncation,
which can be observed from Definition \ref{def:witt:frob}).
...
\begin{rmk}
	...
	various geometric
	variants which are more general than the 
...
we mean component-wise application of these operations.
Use of arbitrary component-wise operations is called
\textit{broadcasting} in Julia, and we will
sometimes use this term.
Homogeneous polynomials are represented in computer 
memory as a list of their terms. 
This is reflected in our notation, 
thus the meaning of ``for \(\delta \in \Delta\)''
is a for loop through all the monomials
of \(\Delta\).

\begin{defn}
    \label{def:poly:nota}
    The \textit{exponent tuple} of a monomial 
    $m = ax_{1}^{d_1} \cdots x_{n}^{d_n}$ is $(d_1, \dots, d_n)$. 
...
\begin{prop}
	...
	\[
		\frac{\tilde{f}^{p} - \sum_{I}^{} ([a_{I}]\mathbf{x}^{I})^{p} }{p}
	\] 
	mod \(p\).
\end{prop}
...
\begin{proof}
    ...
    Witt polynomial \(S_{1}\) (see Remark \ref{rmk:polyraise:w2})
...
\begin{rmk}
    The computation of \(\Delta_{1}(f)\) 
    requires doing arithmetic in \(W_{2}(S)\),
    which is \(\mathbb{Z} / p^{2} \mathbb{Z} [x_{1},\ldots, x_{n}]\) 
    if \(k = \mathbb{F}_{p}\).

    In practice, we represent elements of \(\mathbb{F}_{p}\)
    and \(\mathbb{Z} / p^{2} \mathbb{Z}\) with 
    machine integers periodically reduced modulo $p^2$.
    This way, in computer memory, choosing a lift does
    not change the memory allocation, only the behavior of arithmetic.
\end{rmk}

\begin{rmk}
    \label{rmk:lift:roi}
    For \(k = \mathbb{F}_{q}\),
    we may perform similar computations for 
    \(q \neq p\) by replacing machine integers with
    elements of \(\mathbb{Z}[\zeta_{q-1}]\).
    ...
    so we may lift \(f\) to \(\mathcal{O}_{K}\) 
    and do arithmetic there. 
    Doing arithmetic in a degree \(e\) extension will have
    better complexity than in the cyclotomic extension, which
...
$\tilde{f} \gets \texttt{lift\_coefficients}(f,\mathbb{Z} / p^{2} \mathbb{Z})$\;
$D \gets \tilde{f}^p$\;
...
$D \gets \texttt{divide\_by\_p\_and\_reduce\_mod\_p}(D)$
...
We will use perspective (1) from
...
these monomials, i.e.  \[
S = \bigoplus_{\substack{1 \leq j \leq n,\\ 0 \leq i_{j} \leq p-1}}^{} x_{1}^{i_{1}}\cdots x_{n}^{i_{n}} S^{p}
.\] 
...
\For{$t \in \textnormal{\texttt{terms}} (f)$}{
	...
	\If{$\textnormal{\texttt{exps}} (t) \equiv (p-1, \dots, p-1) \mod p$}{
		%$f \gets f - t$
...
\subsection{A Fedder-type criterion for quasi-\(F\)-splitness}
...
is the Frobenius power of the maximal ideal \((x_1, \ldots, x_n)\).
...
For a K3 surface, the height (if finite) is bounded by 10, inclusive.

\begin{ex}
    Let \(p=3\), and \(n = 4\).
    For the following, we use the simplified notation
    \(x \colonequals x_{1}, y \colonequals x_{2}, z \colonequals x_{3}, w \colonequals x_{4}\).
    Now, let
    \(f = x^4 + y^4 + z^4 + w^4 \in S = k[x,y,z,w]\).
    Raising \(f\) to the power \(p-1 = 2\) in \(\mathbb{F}_{3}\), we get
    \[
    ...
    Since the coefficient of \(c\) in \(g\) is zero, we conclude
    by the classical Fedder's criterion (Theorem \ref{thm:fedder:criterion})
    that \(f\) is not \(F\)-split.
    ...
    We raise \(g^3\) in the integers, and subtract by the cube of every term of \(g\).
