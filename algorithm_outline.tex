...
\end{proof}
% FEEDBACK not sure what this means, is the example found
...
\begin{rmk}
    ...
    % FEEDBACK added a newline here, personally think it looks
    % better
    \hfill
    \begin{enumerate}[(1)]
...
\begin{defn}
	...
	\(S / (f)\) is.
\end{defn}
% FEEDBACK what is k? is it important to specify?
...
and
% FEEDBACK what is "is a the free" supposed to be
...
is an element of \(\Hom(R,R^{p})\), which is a
splitting of Frobenius.
% FEEDBACK think there's a grammar error in sentence above
...
\For{\(t \in \text{terms}(f)\) }
    ...
    % FEEDBACK exponent tuple and broadcasting notation from next section would be useful
    % here, do we bring that up here, or do we just keep it like this? 
...
% FEEDBACK Is something missing after "from"
