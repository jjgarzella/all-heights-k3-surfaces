...
We breifly overview the computational obstacales to fast computation
of \(h\).
...
or Julia's bulit-in matrix multiplication, both of which call the same
...
In our implementation of this verison, step (2) takes
[TIME] seconds for \(p = 5\) and [TIME] seconds for \(p = 7\).
...
Instead, we implement polynomial via Fast Fourier Transform (FFT) using
Graphics Processing Unit (GPU) hardware,
allowing us to run the computation in a massively parallel way.
Our GPU implementation of (1) takes [TIME] seconds for \(p = 5\) and [TIME] seconds for \(p = 7\).
...
In our tests, we were able to achieve high throughput of examples,
...
Overe these two primes, our algorithms are roughly two orders of magnitude
