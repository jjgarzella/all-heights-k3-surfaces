...
takes 0.5 seconds for (1) and 0.73 seconds per iteration for (2),
while for \(p = 7\) it takes 126 seconds for (1) and
21 seconds per iteration for (2).
...
0.0005 seconds per iteration for \(p = 5\) and 0.005 seconds per iteration for \(p = 7\).
Meanwhile, calculating the matrix of multiply then split
takes 0.037 seconds for \(p=5\) and 
0.343 seconds for \(p = 7\).
...
the GPU, we also provide a GPU implementation for calculating the matrix
of multiply then split. 
We also use Nvidia's CUBLAS library \cite{nvidia-2024-cublas} to
do the matrix-vector multiplication.
For $p=5$ and $p=7$, calculating the matrix of multiply then split consistently takes around 0.0001
seconds, and each matrix-vector product takes around 0.0002 seconds.
Thus, the algorithm is still bottlenecked by (1).
...
(see Section \ref{subsec:computations}).
...
in various
Julia packages: 
MMPSingularities.jl (version 0.2.0) \cite{mmpsingularities-jl} contains the algorithms
for the quasi-\(F\)-split Fedder's criterion;
GPUFiniteFieldMatrices.jl (version 0.3.1) \cite{gpuffmatrices-jl}
implements matrix multiplication utilities;
GPUPolynomials.jl (version 0.1.1) contains implementations of polynomial multiplication
\cite{gpupolynomials-jl};
finally, CudaNTTs.jl contains GPU-accelerated implementations of NTT (version 1.2.0) \cite{cudantts-jl}.
...
and Fedder's criterion, including the naive implementation
of \cite[Theorem~C]{kty-2022-fedder}.
In Section 3, we describe our modifications 
...
In Section 4, we describe our GPU-based polynomial
powering algorithm, which utilizes incremental FFT-based multiplication using  
the Goldilocks prime.
In Section 5, we describe the considerations we need to keep in mind
to use CUBLAS.
In Section 6, we describe the computation of surfaces of all possible
