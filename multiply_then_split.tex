...
\begin{algorithm}[H]
    ...
    \KwInput{
        $\Delta, M_d, M_{d^\prime}, p$
    }
    \KwNotation{Let $\ell_d = \textnormal{length}(M_d), \ell_{d^\prime} = \textnormal{length}(M_{d^\prime})$}
    ...
    \For{$m \in M_d$}{
        ...
        \For{$\delta \in \Delta$}{
            ...
            \If{$\code{exps}(\delta) + \code{exps}(m) \equiv (p - 1, \dots, p - 1) \mod p$}{
                ...
                $r \gets \code{indexof}(\text{res},M_{d^{\prime}})$\;
                $\text{mat}[r, c] = \code{coeff}(\delta)$
            }
        }
    }
    \Return mat
...
\KwOutput{$\ell_{d^\prime} \times \ell_d $ matrix representing ``multiply then split''}
$\text{mat} \gets \text{zero matrix of size } \ell_{d^\prime} \times \ell_d $\;
% Gave up on the \code{exps} thing here, personally I believe it looks
% better (example in Algorithm 4), but this probably isn't the final 
% version anyways
...
\While{\(1 \leq r ~\text{and}~ l \leq \length(L)\)}{
    ...
    \uIf{\textnormal{cmp < 0}}{
        \(l \gets l + 1\)\;
    }\uElseIf{\textnormal{0 < cmp}}{
        \(r \gets r - 1\)\;
    }\Else{
        matchr \(\gets R[r]\)\;
        numLmatches \(\gets\) the number of adjacent entries in \(L\)\;
        which are equal to \(L[l]\)\;
        \While{$R[r] == \text{matchr} \&\& 1 \leq r$}{
            \For{\(ll \in (l, l+1, \ldots, l+\textnormal{numLmatches} - 1)\)}{
                res \(\gets (\exps(\Delta^{\prime}[ll]) + \exps(M^{\prime}[r]) - (p-1, \ldots, p-1)) / p\)\;
                coeff \(\gets \coeff(\Delta^{\prime}[ll])\)\;
                \(\text{col} \gets \indexof(M^{\prime}[r],M_{d})\)\;
                \(\text{row} \gets \indexof(\text{res},M_{d^{\prime}})\)\;
                \(\text{mat}[\text{row},\text{col}] = \coeff\)\;
            }
        }
        \(r \gets r - 1\)\;
...
\begin{algorithm}[H]
    ...
    \KwInput{$\Delta, M_{d}, M_{d^{\prime}}, p$}
    \KwNotation{Let \(\ell_{d} = \length(M_{d})\), 
    \(\ell_{d^{\prime}} = \length(M_{d^{\prime}})\)}
    \KwOutput{$\ell_{d^{\prime}} \times \ell_{d}$ 
    	matrix representing ``multiply then split''}
    $\text{mat} \gets \text{zero matrix of size } \ell_{d^\prime} \times \ell_d $\;
    \For{$\delta \in \Delta$}{
        mons $\gets generateMatchingMonomials(\delta, p, M_{d})$\;
        \For{$m \in \text{mons}$}{
            $c \gets \indexof(m,M_{d})$\;
			res $\gets (\exps(\delta) + \exps(m) - (p-1, \ldots, p-1)) / p$\;
			$r \gets \indexof(\text{res},M_{d^{\prime}})$\;
            $\text{mat}[r, c] = \coeff(\delta)$\;
        }
    }
    \Return mat
