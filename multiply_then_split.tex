...
\begin{algorithm}[H]
    ...
    \KwNotation{Let $\ell_d = \textnormal{length}(B_d), \ell_{d^\prime} = \textnormal{length}(B_{d^\prime})$}
    ...
    \For{$m \in B_d$}{
        ...
        \For{$\delta \in \Delta$}{
            ...
            \If{$\code{exps}(\delta) + \code{exps}(m) \equiv (p - 1, \dots, p - 1) \mod p$}{
                ...
                $r \gets \code{indexof}(\text{res},B_{d^{\prime}})$\;
...
\While{\(1 \leq r ~\text{and}~ l \leq \length(L)\)}{
    ...
    \uIf{\textnormal{cmp < 0}}{
        ...
        \While{$R[r] == \text{matchr} \&\& 1 \leq r$}{
            \For{\(ll \in (l, \ldots, l+\textnormal{numLmatches} - 1)\)}{
                res \(\gets (\exps(\Delta^{\prime}[ll]) + \exps(B_{d}^{\prime}[r]) - (p-1, \ldots, p-1)) / p\)\;
                coeff \(\gets \coeff(\Delta^{\prime}[ll])\)\;
                \(\text{col} \gets \indexof(B_{d}^{\prime}[r],B_{d})\)\;
                \(\text{row} \gets \indexof(\text{res},B_{d^{\prime}})\)\;
...
\begin{algorithm}[H]
    ...
    $\text{mat} \gets \text{zero matrix of size } \ell_{d^\prime} \times \ell_d $\;
    \For{$\delta \in \Delta$}{
        mons $\gets generateMatchingMonomials(\delta, p, B_{d})$\;
        \For{$m \in \text{mons}$}{
            $c \gets \indexof(m,B_{d})$\;
			res $\gets (\exps(\delta) + \exps(m) - (p-1, \ldots, p-1)) / p$\;
			$r \gets \indexof(\text{res},B_{d^{\prime}})$\;
