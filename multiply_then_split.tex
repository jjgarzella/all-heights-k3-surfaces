...
\begin{defn}
    The exponent tuple of a monomial 
...
\begin{defn}
    ...
    % FEEDBACK We should figure out which definitions
    % to bold terms in. I'm thinking of 3 options:
    % 1. Bold the "main words" of all definitions
    % 2. Bold the "main words" of the definitions where 
    % we introduce our own terminology. (matches, exponent tuples)
    % 3. No bolding
...
\begin{algorithm}[H]
    ...
    % FEEDBACK the meaning of M_d' took me some finding in the beginning of 
    % the section, maybe we need a \begin{notation} or something before this
    % to make the pseudocode easier to read through
...
% FEEDBACK maybe we can have some notation for (p-1, ..., p-1)? 
% it shows up enough that things can seem a lot cleaner without it
	Then \(A + B = 
	(p-1, \ldots, p-1) \mod p\)
	if and only if 
	\(A + B = 
...
when we consider exponent tuples mod $p$ of 
arbitrary monomials \(M\),
it has a unique
match mod \(p\).
...
\begin{cor}
	...
	then 
	\(m(A) \leq_{lex} m(B)\)
    % FEEDBACK shouldn't this be the other way around
\end{cor}
...
\begin{algorithm}[H]
\caption{Matrix of $\theta$: merge-based algorithm}
\label{alg:theta:merge}
\begin{algorithmic}[1]
\State \textbf{Inputs:} \(\Delta, M_{d}, M_{d^{\prime}}, p\)
\State \textbf{Output}: $length(M_{d^{\prime}}) \times length(M_{d})$ 
	matrix representing $\theta$
    % FEEDBACK If we are deciding to call this theta, then we should go back
    % and replace a lot of "multiply then split"'s with theta.
...
\begin{algorithm}[H]
    ...
    \For{$\delta \in \Delta$}
        ...
        \For{$m \in mons$}
			...
			\State $r \gets indexof(\text{res},M_{d^{\prime}})$
            % FEEDBACK I'm pretty sure this is a = instead of +=
            \State $\text{mat}[r, c] = coeff(\delta)$
        \EndFor
    \EndFor
    \State \Return mat
...
Again, letting $L_{d} = length(M_d)$ and 
...
% FEEDBACK I think it should be specified somewhere that all of 
% algorithms give huge improvements for when there are a lot of terms, like
% our algorithms are probably slower for finding h(x^4 + y^4 + z^4 + w^4 + x^2y^2)
