...
\begin{enumerate}[(1)]
    \item raising (a lift of) \(g = f^{p-1}\) to the \(p\)-th power 
        (in line 2 of Algorithm \ref{alg:calc:delta1}), and
...
\begin{defn}
    (c.f. \cite{stanley-1997-enumerative}, p. 18)
...
monomial basis of the vector space of \\
$d$-homogeneous 
...
Concretely, this means that $\exps(m) + \exps(m^{\prime}) \equiv (p - 1, \dots, p - 1) \bmod p$.
...
\begin{algorithm}[H]
    ...
    \For{$m \in B_d$}{
        \For{$\delta \in \Delta$}{
            ...
            \If{$\code{exps}(\delta) + \code{exps}(m) \equiv (p - 1, \dots, p - 1) \bmod p$}{
                ...
                $M[\text{row}, \text{col}] \mathrel{+}= \code{coeff}(\delta)$
            }
        }
    }
    \Return $M$
...
\begin{lem}
    ...
    \]
    where $\operatorname{sum}(X)$ is the sum of the elements of tuple $X$, and $m$ is the matching function from Corollary \ref{cor:unique:match}.
\end{lem}
